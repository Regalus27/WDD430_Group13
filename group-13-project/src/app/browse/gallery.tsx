...
export const ProductGallery = (props: {products: Product[]}) => {
    ...
    const filteredProducts = products!.filter(product => {
        ...
        const matchesPrice = (minPrice === "" || product.price_in_cents >= minPrice) && (maxPrice === "" || product.price_in_cents <= maxPrice);
                        ...
                        {paginatedProducts.length > 0 ? (
                            paginatedProducts.map(product => (
                                <Link key={product.product_id} href={`/products/${product.product_id}`} passHref>
                                    <div key={product.product_id} className="border rounded-2xl">
                                        <Image src={product.image_url} alt={product.product_name} width={400} height={200} className={`${isMobile ? 'sm:w-1/2 sm:h-32' : 'w-full h-84 object-cover'} mb-5 rounded-2xl overflow-hidden shadow-md transform transition-all duration-300 hover:scale-110 `} />

                                        <div className="px-2">
                                            <span className="text-sm bg-gray-200 px-2 py-1 rounded-md">{product.category}</span>
                                            <div className="flex items-center justify-between mt-2">
                                                <h3 className={`${isMobile ? 'text-sm' : 'text-lg'} font-semibold `}>{product.product_name}</h3>
                                                <p className={`${isMobile ? 'text-sm' : 'text-lg'} font-bold `}>${product.price_in_cents}</p>
                                            </div>
